...
sim_results2 <- tibble(d2 = sim_results$trad$d2, v2 = sim_results$trad$v2, d1 = sim_results$ours$d1, v1 = sim_results$ours$v1, m = sim_results$trad$j_count, n = sim_results$trad$k_count, var_block = sim_results$trad$var_block, var_inter = sim_results$trad$var_inter)
...
sim_results2 %>% 
  ...
  select(-c(d1, d2)) %>% 
  cbind( (sim_results2 %>% group_by(m, n, var_block) %>% summarise_at(c("v1", "v2"), mean) %>% ungroup() %>% select(v1, v2))) %>% 
